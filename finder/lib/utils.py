...
def roles(id):  
 return str(len(requests.get(f"https://groups.roblox.com/v1/groups/{id}/roles").json()['roles']))

def make_embed(group_info, funds=None):
    return dict(
      title="Claimable Group Found!",
        fields=[
         dict(name="**Group Info**", 
value=f"**Name**: `{group_info['name']}`\n**MemberCount**: `{group_info['memberCount']}`\n\n"),
         dict(name="**Group funds**", value=f"**Robux**: `{f'{funds}' if funds is not None else '0'}`"),
